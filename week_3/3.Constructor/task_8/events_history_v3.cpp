...
public:
    ...
    string GetFullName(const int& year) const {
        if (allEvents.empty() || year < allEvents.begin()->first) return "No person";
    ...
    string GetFullNameWithHistory(int year) const {
        if (allEvents.empty() || minYear != 0 && minYear > year) return "No person";
        else if (year < allEvents.begin()->first) return "No person";
